...
This Pluto notebook implements an interactive thermometer state machine using HierarchicalStateMachines.jl. The assumption is that you are familiar with hierarchical state machines, and the HierarchicalStateMachines.jl library, so the annotations in this notebook are minimal.

If you're unfamiliar with hierarchical state machines in general, or HierarchicalStateMachines.jl in particular? Try reading
...
with_terminal() do
	with_logger(logger) do
		HSM.handle_event!(thermometer_state_machine, TemperatureEvent(new_temp))
		println("display: $(thermometer.display)")
		println("temperature: $(thermometer.temperature)")
	end
end
...
manifest_format = "2.0"
project_hash = "d286b79ba1d61e37e831a8fb90e073c1913fd738"
